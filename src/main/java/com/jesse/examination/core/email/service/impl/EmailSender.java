...
public class EmailSender implements EmailSenderInterface
{
     ...
     * 返回一个承载了上述两个信息的元组。
     */
    private Mono<Tuple2<String, String>>
        ...
        return Mono.fromCallable(() -> {
            try {
                ...
                else
                {
                    ...
                    if (content.getAttachmentPath() != null)
                    {
                        File attachment = new File(content.getAttachmentPath());

                        if (!attachment.exists()) {
                            throw new EmailException(
                                EmailException.ErrorType.INVALID_CONTENT,
                                "Attachment file not found!", null
                            );
                        }
                ...
                // log.info("Email send to: {} complete.", content.getTo());
